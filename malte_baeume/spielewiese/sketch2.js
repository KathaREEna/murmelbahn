...
function setup() {
  ...
  const boden = (new BlockCore(
    world,
    {
      x: 0.5*windowWidth, y: 1.3*windowHeight, w: 1.5*windowWidth, h: 1*windowHeight,
      color: 'darkblue',
      rotate: { angle: 0, delta: 0.001 }
    },
    {  angle: -PI / 1.05, isStatic: true }
    
  ));
  blocks.push(boden)
  console.log(boden.attrs.rotate.delta)
  boden.attrs.rotate.delta = -0.001
  // blocks.push(new BlockCore(
  //   world,
  //   {
  ...
  blocks.push(new Ball(
    world,
    {
      x: windowWidth/2, y: 0, r: 30,
      color: 'white'
    },
    { label: "Murmel", restitution: 0.6, friction: 0 }
  ));
    // the ball has a label and can react on collisions
    blocks.push(new Ball(
      world,
      {
        x: windowWidth/2+200, y: 0, r: 15,
        color: 'white'
      },
      { restitution: 0.6, friction: 0 }
    ));
      // the ball has a label and can react on collisions
  blocks.push(new Ball(
    world,
    {
      x: windowWidth/2-500, y: 0, r: 15,
      color: 'white'
    },
    {  restitution: 0.6, friction: 0 }
  ));
